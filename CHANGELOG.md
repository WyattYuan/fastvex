...
- 优化 `migrate` 迁移命令的默认行为：现在默认在原地应用迁移并就地备份，避免生成带 `v2` 后缀的新文件后再手动改名；使用 `--in-place` 取代原版繁琐的 `--write` 标志。

## [0.1.2] - 2026-05-29

### 修复

- 修复源码运行和 zipapp 分发场景下版本号获取失败导致启动崩溃的缺陷
- 修复 Windows 特定 PATH 环境下外部工具调用时异常处理不完整的问题
- 修复部署结果面板标题缺少状态标识、与详情区域视觉不一致的问题
- 修复 `--dry-run` 模式下误检 PROS 工具链导致无法正常预览部署计划的问题
- 提升异常处理健壮性，避免非预期错误被静默吞掉
